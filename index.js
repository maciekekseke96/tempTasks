...
//FIND ALL POSTS FOR EVERY USER AND COUNT POSTS FOR EVERY USER

let usersWithPosts = [];

const findPostsById = (user, postsArray, commentsArray) => {
  let newUser = { ...user };
  let postsWithComments = [];

  const postsFound = postsArray.filter((post) => {
  ...
  postsFound.forEach((post) => {
    ...
    const commentsFound = commentsArray.filter((comment) => {
    ...
    commentsFound.forEach((comment) => {
      let commentWithoutId = { ...comment };
      delete commentWithoutId.id;
      newCommentsObject = {
        ...newCommentsObject,
        [comment.id]: commentWithoutId,
      };
    });

    const postWithComments = {
      ...newPost,
      comments: newCommentsObject,
      commentsAmount: Object.keys(newCommentsObject).length,
    };
  ...
  const userWithPosts = {
    ...
    postsAmmount: postsWithComments.length,
  };
...
usersResponse.forEach((user) => {
  const userWithPosts = findPostsById(user, postsResponse, commentsResponse);
